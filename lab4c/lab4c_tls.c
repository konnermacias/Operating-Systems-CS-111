...
int main(int argc, char** argv) {
      		...
      		case 'p':
                ...
                case 's':
			        ...
			        exit(1);
                        ...
                        break;
		default:
    			fprintf(stderr, "Invalid option. Correct Usage: ./lab4c_tls portnumber --id=ID-number --host=name --log=filename [--scale=CF] [--period=num_seconds]\n");
    			exit(1);
    		}
  	}
	...
	if (SSL_library_init() < 0) {
		errorMe("using SSL_library_init()");
	}
	SSL* ssl_client = 0;	
	SSL_load_error_strings();
	OpenSSL_add_all_algorithms();
	SSL_CTX* context = SSL_CTX_new(TLSv1_client_method());
	if ((ssl_client = SSL_new(context)) == NULL) {
		errorMe("using SSL_new()");
	}
	if ((SSL_set_fd(ssl_client, sockFd)) == 0) {
		errorMe("using SSL_set_fd()");
	}
	if (SSL_connect(ssl_client) != 1) {
		errorMe("using SSL_connect()");
	}
	...
	while(flag) {
		...
		/* received input via terminal */
	    	if ((pfd[0].revents&POLLIN) == POLLIN) {
	        	/* get input */
	        	if ((readRes = SSL_read(ssl_client, readBuf, BUF_SIZE)) == -1 ) {
	          		errorMe("reading");
	        	} else if (readRes == 0) {
